...
function prepMessage() {
  ...
  const dbRefType = firebase.database().ref().child("Portfolio").child("count");

  dbRefType.once('value', function(snapshot) { 
  var numberMessages = (parseInt(snapshot.val()) + 1);
  console.log("number of messages: " + numberMessages);
  uploadMessage(numberMessages,name,email,subject,message);
        });
}

function uploadMessage(numberMessages,name,email,subject,message) {
    firebase.database().ref('Portfolio/' + 'Messages/' + 'Message_' + numberMessages).set({
      name: name,
      email: email,
      subject: subject,
      content: message
    ...
    var updates = {};
    updates['Portfolio/' + "count"] = numberMessages;
    firebase.database().ref().update(updates); 
    
    document.getElementById('result').innerHTML = "Message Uploaded!";
}